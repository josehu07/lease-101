...
- Light page theme with a dark-blue banner (its own `--nav-*` palette). CSS custom properties for the rest of the palette (`--grantor` orange, `--grantee` green to mirror Figure 2 in the paper).
- **Responsive.** The desktop layout is capped to `--max-width` (820px) and centered. Two phone breakpoints refine it:
  - `max-width: 600px` — the banner's brand + 5-link row no longer fits one line (needs ~570px), so `.nav-inner` wraps and `.nav-links` becomes a full-width, wrapping second row (never clips off-screen); page gutters and section spacing tighten; and the root font shrinks to **14px** (from the 17px desktop base). Because every scenario-canvas overlay (nodes, timer bars, crown) is **rem-sized** while the `.pg-stage` box is viewport-driven, the smaller rem scales those overlays down to fit a narrow stage — the ring-fit math (`sim_view.rs`) assumes a conservative ~520px box, which a phone stage falls below, so without this the busy-topology timer stacks poke past the stage edge.
  - `max-width: 560px` — playground control rows stack (label over control), the pro/con `.tradeoff` pair stacks to one column, and the scenario-figure box padding tightens.
- `body` sets `overflow-x: clip` (not `hidden`, so the sticky nav still pins) as a belt-and-suspenders guard against horizontal scroll from decorative absolutely-positioned overflow — chiefly the hidden `.pg-tconst` / `.pg-hint` hover tooltips, which are laid out `nowrap` and can extend past a narrow viewport's right edge. A no-op on desktop, where content is centered far from the edges.
